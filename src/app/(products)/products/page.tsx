...
export default function ProductsPage() {
  const { filters, setFilters, getFilteredProducts, quickViewProduct, setQuickViewProduct } = useProductStore();
                      ...
                      <div>
                        <div className="absolute top-3 right-3 z-10">
                          <button
                            ...
                            className="p-1.5 rounded-full bg-black/70 backdrop-blur border border-white/8 text-zinc-400 hover:text-red-500 transition-colors"
                          >
                        ...
                        <div className="w-full h-44 bg-zinc-900 rounded-t-2xl overflow-hidden relative mb-4 flex items-center justify-center">
                          ...
                          <div className="absolute inset-0 bg-black/65 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-3">
                            <button
                              ...
                              className="p-2.5 rounded-full bg-zinc-950/80 backdrop-blur border border-white/8 text-zinc-300 hover:text-primary-gold hover:border-primary-gold transition-all"
                              title="Quick View"
                            >
                              <Eye size={16} />
                            </button>
                    ...
                    >
                      ...
                      <div className="flex-1 space-y-2 min-w-0 pr-6">
                        ...
                        <div className="flex space-x-3 pt-2">
                          <Link href={`/products/${product.id}`}>
                            <button className="btn-outline px-5 py-2 text-[10px] font-bold uppercase tracking-wider">
                              Inspect Details
                            </button>
                          </Link>
                          <button
                            className="btn-gold px-5 py-2 text-[10px] font-bold uppercase tracking-wider"
                            onClick={() => {
                              addItem(product, 1, product.colors[0], product.storage[0]);
                              showToast(`${product.name} added to cart!`, 'success');
                            }}
